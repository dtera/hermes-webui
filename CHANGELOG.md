...
- **PR #2249** by @franksong2702 (fixes #2248, follow-up to #2244) — Session Pin, Rename, and Personality changes now reload cached metadata-only sessions before mutating and saving persisted fields. This matches the Archive Session metadata-only cache fix while preserving the #1558 `Session.save()` guard that refuses direct saves of metadata-only stubs. Regression coverage forces a metadata-only session into the in-memory cache for all three routes and verifies the saved session keeps its original messages while the cache is upgraded to a full session.
