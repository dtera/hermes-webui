...
- Sidebar session rows whose persisted JSON title is a stale generic `Hermes WebUI #N` now use the fresher same-session `state.db` title for read-only display/search. The persisted JSON title is left unchanged, custom/manual titles remain authoritative, and default numeric WebUI titles keep their `#N` suffix instead of being treated as hashtag filters.
